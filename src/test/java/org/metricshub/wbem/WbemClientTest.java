...
class WbemClientTest {
	...
	void testConnect() throws Exception {
		...
		Assertions.assertThrows(
			...
			() -> new WbemClient().connect(null, username, password, timeout)
		);
		Assertions.assertThrows(
			IllegalArgumentException.class,
			() -> new WbemClient().connect(url, null, password, timeout)
		);
		Assertions.assertThrows(
			IllegalArgumentException.class,
			() -> new WbemClient().connect(url, username, null, timeout)
		);

		try (
			final MockedStatic<WBEMClientFactory> mockedWBEMClientFactory = Mockito.mockStatic(WBEMClientFactory.class);
	...
	void testExecuteWql() throws Exception {
		...
		Assertions.assertThrows(
			IllegalArgumentException.class,
			() -> new WbemClient().executeWql(queryHandler, null, null)
		);

		Assertions.assertThrows(
			IllegalStateException.class,
			() -> new WbemClient().executeWql(queryHandler, namespace, null)
		);
		...
		try (
			...
			mockedMatsyaWbemClient
				.when(
					() ->
						WbemClient.enumerateInstances(
	...
	void testGetAssociators() throws Exception {
		...
		Assertions.assertThrows(
			IllegalArgumentException.class,
			() -> new WbemClient().getAssociators(null, objectPathAssociators, null)
		);
		Assertions.assertThrows(
			IllegalArgumentException.class,
			() -> new WbemClient().getAssociators(queryHandler, null, null)
		);

		Assertions.assertThrows(
			IllegalStateException.class,
			() -> new WbemClient().getAssociators(queryHandler, objectPathAssociators, null)
		);
