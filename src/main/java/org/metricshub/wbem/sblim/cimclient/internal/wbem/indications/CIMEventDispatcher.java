...
public class CIMEventDispatcher extends Thread {
	...
	public CIMEventDispatcher(EventListener pListener, int pMaxEvents) {
		if (
			!(pListener instanceof IndicationListener) && !(pListener instanceof IndicationListenerSBLIM)
	...
	public synchronized void dispatchEvent(CIMEvent pEvent) {
		if (pEvent != null) {
				...
				if (size >= this.iMaxEvents) {
					for (int i = size - this.iMaxEvents + 1; i > 0; i--) {
						...
						this.iLogger.trace(
								...
								"Deleted CIMEvent (id=" +
								event.getID() +
								") from the queue (maximum size of " +
								this.iMaxEvents +
								" reached)"
							);
					}
			...
			this.iLogger.trace(
					Level.FINE,
					"Added CIMEvent (id=" + pEvent.getID() + ") to the queue (" + this.iEventQueue.size() + " elements total)"
				);
			notify();
		} else {
	...
	private synchronized CIMEvent getEvent() {
		...
		while (event == null) {
			...
			if (this.iEventQueue.size() > 0) {
				...
				this.iLogger.trace(
						Level.FINER,
						"Removed CIMEvent (id=" + event.getID() + ") from the queue (" + this.iEventQueue.size() + " elements left)"
					);
			}
		}
		return event;
		...
		while (this.iAlive) {
			try {
				...
				if (event != null) {
					...
					try {
						if (this.iListener instanceof IndicationListener) ((IndicationListener) this.iListener).indicationOccured(
								event.getID(),
								event.getIndication()
							); else ((IndicationListenerSBLIM) this.iListener).indicationOccured( // if instanceof IndicationListenerSBLIM)
								event.getID(),
								event.getIndication(),
								event.getInetAddress()
							);
					} catch (Throwable t) {
						this.iLogger.trace(
								Level.FINE,
								"Exception caught in listener (" + this.iListener.getClass().getName() + ") while processing CIMEvent",
								t
							);
					}
				}
